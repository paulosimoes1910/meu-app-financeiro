...
import { LayoutDashboard, Plus, List, CreditCard, Repeat, Calendar, XCircle, Plane, Clock, Menu, X, ArrowUp, ArrowDown, CheckCircle, Trash2, AlertTriangle, TrendingUp, TrendingDown, ChevronLeft, ChevronRight, Eye, EyeOff, Archive, FileDown } from 'lucide-react';

// --- Configuração do Firebase (VERSÃO CORRETA PARA O SEU PROJETO) ---
...
// --- Componente Modal de Confirmação para Deletar ---
...
// --- Componente de Conteúdo da Visão Geral ---
const VisaoGeralContent = ({ transacoes, cartoes, prestacoes, formatCurrency, isPdfLibReady }) => {
    ...
    const monthlySummary = useMemo(() => {
        ...
        const filteredCartoes = cartoes.filter(c => {
            const cDate = new Date(c.vencimentoFatura + 'T00:00:00Z');
            return cDate && cDate.getUTCFullYear() === year && cDate.getUTCMonth() === month;
        });

        const filteredPrestacoes = prestacoes.filter(p => {
            const startDate = p.data?.toDate() || p.createdAt?.toDate() || new Date();
            const monthsPassed = (year - startDate.getFullYear()) * 12 + (month - startDate.getMonth());
            return monthsPassed >= 0 && monthsPassed < p.numParcelas;
        });

        const totalReceitas = filteredTransacoes.filter(t => t.tipo === 'receita').reduce((acc, t) => acc + t.valor, 0);
        const despesasGerais = filteredTransacoes.filter(t => t.tipo === 'despesa').reduce((acc, t) => acc + t.valor, 0);
        const faturasCartao = filteredCartoes.reduce((acc, c) => acc + c.valorFatura, 0);
        const prestacoesMes = filteredPrestacoes.reduce((acc, p) => acc + p.valorParcela, 0);
        
        const totalDespesas = despesasGerais + faturasCartao + prestacoesMes;
        const saldoTotal = totalReceitas - totalDespesas;

        const prestacoesAtivas = prestacoes.map(p => {
            const startDate = p.data?.toDate() || p.createdAt?.toDate() || new Date();
            const monthsPassed = (currentDate.getFullYear() - startDate.getFullYear()) * 12 + (currentDate.getMonth() - startDate.getMonth());
            return { ...p, isAtiva: monthsPassed >= 0 && monthsPassed < p.numParcelas, parcelaAtual: monthsPassed + 1 };
        }).filter(p => p.isAtiva);
    ...
    const generatePDF = () => {
        ...
        const addTableWithTotal = (title, head, body, totalLabel, totalValue) => {
            if (body.length > 0) {
                ...
                const x = pageWidth - doc.autoTable.previous.margin.right - textWidth;
        ...
        addTableWithTotal("Receitas", ["Descrição", "Data", "Valor"], receitasDoMes.map(item => [item.descricao, item.data?.toDate().toLocaleDateString('pt-BR') || 'N/A', formatCurrency(item.valor)]), "Total Receitas", monthlySummary.totalReceitas);

        const despesasDoMes = monthlySummary.filteredTransacoes.filter(t => t.tipo === 'despesa');
        addTableWithTotal("Despesas", ["Descrição", "Data", "Valor"], despesasDoMes.map(item => [item.descricao, item.data?.toDate().toLocaleDateString('pt-BR') || 'N/A', formatCurrency(item.valor)]), "Total Despesas", monthlySummary.despesasGerais);

        addTableWithTotal("Faturas de Cartão", ["Nome", "Vencimento", "Valor"], monthlySummary.filteredCartoes.map(item => [item.nome, new Date(item.vencimentoFatura + 'T00:00:00Z').toLocaleDateString('pt-BR'), formatCurrency(item.valorFatura)]), "Total Faturas", monthlySummary.faturasCartao);

        addTableWithTotal("Prestações do Mês", ["Descrição", "Valor da Parcela"], monthlySummary.filteredPrestacoes.map(item => [item.descricao, formatCurrency(item.valorParcela)]), "Total Prestações", monthlySummary.prestacoesMes);
        
        finalY = Math.max(finalY, 20); // Garante que o resumo não sobreponha as tabelas
        doc.setFontSize(12);
        doc.setFont(undefined, 'normal');
        doc.text("Resumo Financeiro do Mês", 14, finalY);
        doc.autoTable({
            body: [
                ['Total de Receitas:', formatCurrency(monthlySummary.totalReceitas)],
                ['Total de Despesas:', formatCurrency(monthlySummary.totalDespesas)],
                ['SALDO FINAL:', formatCurrency(monthlySummary.saldoTotal)],
            ],
            startY: finalY + 6,
            theme: 'striped',
        ...
        <div className="p-4 sm:p-6 space-y-6">
            <div className="flex justify-between items-center bg-gray-800 p-4 rounded-lg">
                 ...
                 <div className="text-center">
                     <p className="text-lg font-bold text-white">{currentDate.toLocaleString('pt-BR', { month: 'long', year: 'numeric' }).toUpperCase()}</p>
                 </div>
                 <button onClick={handleNextMonth} className="p-2 rounded-full hover:bg-gray-700"><ChevronRight size={24} /></button>
                 <button onClick={generatePDF} disabled={!isPdfLibReady} className="p-2 bg-blue-600 hover:bg-blue-500 rounded-lg transition-colors flex items-center gap-2 disabled:bg-gray-500 disabled:cursor-not-allowed">
                     <FileDown size={20} />
                     <span className="hidden sm:inline">{isPdfLibReady ? 'Relatório' : 'A carregar...'}</span>
                 </button>
            </div>

            <div className="bg-blue-600 rounded-2xl p-6 text-white shadow-lg flex flex-col justify-between min-h-[180px] relative">
                <button onClick={toggleVisibility} className="absolute top-4 right-4 text-blue-200 hover:text-white transition-colors">
                    {isDataVisible ? <EyeOff size={20} /> : <Eye size={20} />}
                </button>
                <div className="text-center">
                    <h2 className="text-sm font-semibold text-blue-200">Saldo do Mês</h2>
                    <p className={`text-4xl font-bold mt-1 ${monthlySummary.saldoTotal < 0 ? 'text-red-300' : ''}`}>{isDataVisible ? formatCurrency(monthlySummary.saldoTotal) : hiddenValue}</p>
                </div>
                <div className="flex justify-between items-center">
                    <div className="flex items-center gap-2">
                        <div className="bg-white/20 p-2 rounded-full"><TrendingUp size={16} /></div>
                        <div>
                            <p className="text-xs text-blue-200">Receitas</p>
                            <p className="font-semibold">{isDataVisible ? formatCurrency(monthlySummary.totalReceitas) : hiddenValue}</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <div className="bg-white/20 p-2 rounded-full"><TrendingDown size={16} /></div>
                        <div>
                            <p className="text-xs text-blue-200">Despesas</p>
                            <p className="font-semibold">{isDataVisible ? formatCurrency(monthlySummary.totalDespesas) : hiddenValue}</p>
                        </div>
                    </div>
                </div>
            </div>
            <div className="bg-gray-800 rounded-2xl p-6">
                <h3 className="text-lg font-bold text-white mb-4">Divisão de Despesas do Mês</h3>
                <div className="space-y-2">
                    <div className="flex justify-between items-center">
                        <div className="flex items-center gap-3"><span className="h-3 w-3 rounded-full bg-red-500"></span><p>Despesas Gerais</p></div>
                        <p className="font-semibold text-red-400">{isDataVisible ? formatCurrency(monthlySummary.despesasGerais) : hiddenValue}</p>
                    </div>
                    <div className="flex justify-between items-center">
                        <div className="flex items-center gap-3"><span className="h-3 w-3 rounded-full bg-blue-400"></span><p>Faturas de Cartão</p></div>
                        <p className="font-semibold text-blue-400">{isDataVisible ? formatCurrency(monthlySummary.faturasCartao) : hiddenValue}</p>
                    </div>
                    <div className="flex justify-between items-center">
                        <div className="flex items-center gap-3"><span className="h-3 w-3 rounded-full bg-yellow-400"></span><p>Prestações do Mês</p></div>
                        <p className="font-semibold text-yellow-400">{isDataVisible ? formatCurrency(monthlySummary.prestacoesMes) : hiddenValue}</p>
                    </div>
                </div>
            </div>
            <div className="bg-gray-800 rounded-2xl p-6">
                <h3 className="text-lg font-bold text-white mb-4">Acompanhamento de Prestações (Geral)</h3>
                <div className="space-y-4">{monthlySummary.prestacoesAtivas.length > 0 ? monthlySummary.prestacoesAtivas.map(p => (<div key={p.id}><div className="flex justify-between items-center mb-1"><p className="font-semibold">{p.descricao}</p><p className="text-sm text-gray-400">Parcela {p.parcelaAtual} de {p.numParcelas}</p></div><div className="w-full bg-gray-700 rounded-full h-2.5"><div className="bg-yellow-500 h-2.5 rounded-full" style={{ width: `${(p.parcelaAtual / p.numParcelas) * 100}%` }}></div></div></div>)) : <p className="text-sm text-gray-500">Nenhuma prestação ativa este mês.</p>}</div>
            </div>
        </div>
    );
};
        ...
        <div className="max-w-4xl mx-auto p-4 sm:p-6">
            ...
            <div className="bg-gray-800 rounded-2xl p-6">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="text-center">
                        <p className="text-lg font-bold">{currentCycleInfo.monthLabel}</p>
                        <p className="text-sm text-gray-400">{currentCycleInfo.year}</p>
                    </div>
                ...
                <div className="grid grid-cols-7 gap-2 text-center text-xs font-bold text-gray-400 mb-2">
                    {['SEG', 'TER', 'QUA', 'QUI', 'SEX', 'SÁB', 'DOM'].map((day, index) => (
                        <span key={day} className={index >= 5 ? 'text-blue-400' : ''}>{day}</span>
                    ))}
                </div>
                <div className="grid grid-cols-7 gap-2">
                    {currentCycleInfo.days.map((day, index) => {
                        ...
                        if (isWeekend && !isToday && !isHoraExtra && !isFerias && !isFalta) {
                            dayClasses += ' bg-gray-700/40';
                        }
                        
                        if (isFerias) { dayClasses += ' bg-teal-600 text-white font-bold'; }
                        else if (isFalta) { dayClasses += ' bg-orange-600 text-white font-bold'; }
                        else if (isHoraExtra) { dayClasses += ' bg-indigo-600 text-white font-bold'; }
...
// --- Componente de Conteúdo da Aba Gastos Fixos ---
const GastosFixosContent = ({ gastosFixos, formatCurrency, handleDeleteRequest, isPdfLibReady }) => {
    ...
    const generatePDF = (gastos, filter) => {
        ...
        const totalParcelasMes = gastos.reduce((acc, item) => acc + item.valorParcela, 0);
        const dividaTotal = gastos.reduce((acc, item) => acc + item.valorTotal, 0);

        doc.text(`Total Parcelas do Mês: ${formatCurrency(totalParcelasMes)}`, 14, finalY + 16);
        doc.text(`Dívida Total: ${formatCurrency(dividaTotal)}`, 14, finalY + 22);
    ...
    const filterOptions = useMemo(() => {
        const descriptions = gastosFixos.map(item => item.descricao);
        return ['todos', ...new Set(descriptions)];
    }, [gastosFixos]);
    ...
    const filteredGastos = useMemo(() => {
        ...
        return gastosFixos.filter(item => item.descricao === filter);
    }, [filter, gastosFixos]);
    ...
    const summary = useMemo(() => {
        ...
        const gastosFixosAtivos = filteredGastos.map(p => {
            const startDate = p.data?.toDate() || p.createdAt?.toDate() || new Date();
            const monthsPassed = (currentYear - startDate.getFullYear()) * 12 + (currentMonth - startDate.getMonth());
            return { ...p, isAtiva: monthsPassed >= 0 && monthsPassed < p.numParcelas, parcelaAtual: monthsPassed + 1 };
        }).filter(p => p.isAtiva);
    ...
    return (
        <div className="max-w-4xl mx-auto p-4 sm:p-6">
            <div className="bg-gray-800 rounded-2xl p-6 mb-6">
                <h3 className="text-lg font-bold text-white mb-4 text-center">Contabilidade de Gastos Fixos</h3>
                <div className="flex justify-center items-end gap-12 text-center">
                    <div><p className="text-2xl font-bold text-purple-400">{formatCurrency(summary.gastosFixosMes)}</p><p className="text-sm text-gray-400">Total Parcelas do Mês</p></div>
                    <div><p className="text-base font-bold text-gray-300">{formatCurrency(summary.totalDividaFixa)}</p><p className="text-sm text-gray-400">Dívida Total</p></div>
                </div>
            </div>
                ...
                <div className="flex-grow">
                    ...
                    <select id="filter-gastos" value={filter} onChange={(e) => setFilter(e.target.value)} className="w-full p-3 bg-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500">
                        {filterOptions.map(option => (
                            <option key={option} value={option}>{option === 'todos' ? 'Todos os Lançamentos' : option}</option>
                        ))}
                    </select>
                </div>
                <button onClick={() => generatePDF(filteredGastos, filter)} disabled={!isPdfLibReady} className="p-3 bg-purple-600 hover:bg-purple-500 rounded-lg transition-colors flex items-center gap-2 disabled:bg-gray-500 disabled:cursor-not-allowed">
                    <FileDown size={20} />
                    <span>{isPdfLibReady ? 'Relatório' : 'A carregar...'}</span>
                </button>
            </div>

            <h2 className="text-2xl font-bold mb-4">Relatório de Gastos Fixos</h2>
            <div className="space-y-3">
                {filteredGastos.length > 0 ? filteredGastos.map(item => (
                    <div key={item.id} className="bg-gray-800 p-4 rounded-lg flex items-center">
                        <div className="flex-grow">
                            ...
                            <p className="text-xs text-gray-400">{item.numParcelas} parcelas de {formatCurrency(item.valorParcela)}</p>
                        </div>
                        <div className="text-right mr-6">
                            <p className="text-sm text-gray-400">Total</p>
                            <p className="font-bold text-lg text-purple-400">{formatCurrency(item.valorTotal)}</p>
                        </div>
                        <button onClick={() => handleDeleteRequest(item.id, 'gasto_fixo')} className="text-gray-600 hover:text-red-500 transition-colors">
                            <Trash2 size={20} />
                        </button>
                    </div>
                )) : <p className="text-gray-500 text-center mt-8">Nenhum gasto fixo encontrado.</p>}
            </div>
        </div>
    );
};
...
export default function App() {
    const [isMenuOpen, setIsMenuOpen] = useState(false); const [activeTab, setActiveTab] = useState('Visão Geral'); const [userId, setUserId] = useState(null); const [transacoes, setTransacoes] = useState([]); const [cartoes, setCartoes] = useState([]); const [prestacoes, setPrestacoes] = useState([]); const [gastosFixos, setGastosFixos] = useState([]); const [faltas, setFaltas] = useState([]); const [ferias, setFerias] = useState([]); const [horasExtras, setHorasExtras] = useState([]); const [modalType, setModalType] = useState(null); const [isSuccessVisible, setIsSuccessVisible] = useState(false); const [itemToDelete, setItemToDelete] = useState(null); const [alertInfo, setAlertInfo] = useState({ isOpen: false, title: '', message: '' });
    const [isPdfLibReady, setIsPdfLibReady] = useState(false);

    const formatCurrency = (value) => { const numberValue = typeof value === 'number' ? value : 0; return numberValue.toLocaleString('en-GB', { style: 'currency', currency: 'GBP' }); };
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            if (document.head.contains(jspdfAutotableScript)) {
                document.head.removeChild(jspdfAutotableScript);
            }
        }
    }, []);
    ...
    useEffect(() => {
        ...
        const getItemDate = (item) => {
            ...
            if (item.data) return new Date(item.data + 'T00:00:00Z'); 
            if (item.dataInicio) return new Date(item.dataInicio + 'T00:00:00Z');
            if (item.vencimentoFatura) return new Date(item.vencimentoFatura + 'T00:00:00Z');
            return new Date(0);
        };
    ...
    const handleLaunchTypeSelect = (type) => { if (['receita', 'despesa', 'cartao', 'prestacao', 'gasto_fixo', 'falta', 'ferias', 'hora_extra'].includes(type)) { setModalType(type); } else { alert(`O formulário para "${type}" ainda será implementado.`); } };
    const handleDeleteRequest = (id, type) => { setItemToDelete({ id, type }); };
    const handleConfirmDelete = async () => {
        if (!itemToDelete || !userId || !db) return;
        ...
        switch (type) {
            ...
            default: console.error("Tipo de item para deletar desconhecido:", type); return;
        }
        try {
        ...
        return (
            <div className="bg-gray-900 text-white min-h-screen flex items-center justify-center p-4">
                <div className="text-center bg-gray-800 p-8 rounded-lg">
                    ...
                    <p className="text-gray-400 mt-2">A configuração do Firebase não foi encontrada. O aplicativo não pode ser carregado.</p>
                </div>
            </div>
        )
    }
    ...
    const renderActiveTabContent = () => {
        switch (activeTab) {
            ...
            default: return <div className="p-6"><h2 className="text-2xl font-bold">{activeTab}</h2><p className="mt-2 text-gray-400">Conteúdo em desenvolvimento.</p></div>;
        }
    };

    const MainMenu = () => (<div className="fixed inset-0 bg-gray-900 z-50 p-6 flex flex-col"><div className="flex justify-between items-center mb-8"><h2 className="text-2xl font-bold text-white">Menu Principal</h2><button onClick={() => setIsMenuOpen(false)} className="text-gray-400 hover:text-white"><X size={32} /></button></div><nav className="flex flex-col gap-1"><MenuItem icon={<LayoutDashboard size={24} />} label="Visão Geral" isTab /><MenuItem icon={<Plus size={24} />} label="Lançamentos" isTab /><MenuItem icon={<List size={24} />} label="Transações" isTab /><MenuItem icon={<CreditCard size={24} />} label="Cartões" isTab /><MenuItem icon={<Repeat size={24} />} label="Prestações" isTab /><MenuItem icon={<Archive size={24} />} label="Gastos Fixos" isTab /><MenuItem icon={<Calendar size={24} />} label="Calendário" isTab /><p className="text-gray-500 font-bold text-sm mt-4 mb-2 px-4">CONTROLE DE PONTO</p><MenuItem icon={<XCircle size={24} />} label="Faltas" isTab /><MenuItem icon={<Plane size={24} />} label="Férias" isTab /><MenuItem icon={<Clock size={24} />} label="Horas Extras" isTab /></nav><div className="mt-auto text-center text-xs text-gray-600"><p>ID do Usuário:</p><p className="break-all">{userId || 'Autenticando...'}</p></div></div>);
    const MenuItem = ({ icon, label, isTab }) => { const isActive = activeTab === label; return (<button onClick={() => { if (isTab) { setActiveTab(label); } setIsMenuOpen(false); }} className={`w-full text-left flex items-center gap-4 px-4 py-2 rounded-lg text-lg transition-all duration-200 ${isActive ? 'bg-white/10 text-white font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}>{icon}<span>{label}</span></button>); };

    return (
        <div className="bg-gray-900 text-white min-h-screen font-sans" style={{ fontFamily: "'Poppins', sans-serif" }}>
            <style>{`
                @import url('https://fonts.googleapis.com/css2?family=Poppins:wght@400;600;700&display=swap');
                .animate-fade-in { animation: fadeIn 0.3s ease-in-out; }
                @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }
                .animate-slide-down-fade { animation: slideDownFadeOut 3s ease-in-out forwards; }
            ...
            `}</style>
            {isMenuOpen && <MainMenu />}
            <ReceitaModal isOpen={modalType === 'receita'} onClose={() => setModalType(null)} userId={userId} showSuccessMessage={showSuccessMessage} />
            <DespesaModal isOpen={modalType === 'despesa'} onClose={() => setModalType(null)} userId={userId} showSuccessMessage={showSuccessMessage} />
            <CartaoModal isOpen={modalType === 'cartao'} onClose={() => setModalType(null)} userId={userId} showSuccessMessage={showSuccessMessage} />
            <PrestacaoModal isOpen={modalType === 'prestacao'} onClose={() => setModalType(null)} userId={userId} showSuccessMessage={showSuccessMessage} />
            <GastoFixoModal isOpen={modalType === 'gasto_fixo'} onClose={() => setModalType(null)} userId={userId} showSuccessMessage={showSuccessMessage} />
            <FaltaModal isOpen={modalType === 'falta'} onClose={() => setModalType(null)} userId={userId} showSuccessMessage={showSuccessMessage} faltas={faltas} showCustomAlert={showCustomAlert} />
            <FeriasModal isOpen={modalType === 'ferias'} onClose={() => setModalType(null)} userId={userId} showSuccessMessage={showSuccessMessage} />
            <HoraExtraModal isOpen={modalType === 'hora_extra'} onClose={() => setModalType(null)} userId={userId} showSuccessMessage={showSuccessMessage} />
...
const TransacoesContent = ({ transacoes, formatCurrency, handleDeleteRequest, isPdfLibReady }) => {
    ...
    const generatePDF = (items, filter) => {
        ...
        items.forEach(item => {
            const itemData = [
                item.descricao,
                item.data?.toDate().toLocaleDateString('pt-BR') || item.createdAt?.toDate().toLocaleDateString('pt-BR') || 'N/A',
                item.tipo,
        ...
        doc.text(`Total de Receitas: ${formatCurrency(totalReceitas)}`, 14, finalY + 16);
        doc.text(`Total de Despesas: ${formatCurrency(totalDespesas)}`, 14, finalY + 22);
        doc.text(`Saldo: ${formatCurrency(saldo)}`, 14, finalY + 28);

        doc.save(`relatorio_transacoes_${filter}.pdf`);
    };

    const filterOptions = useMemo(() => {
        const descriptions = transacoes.map(item => item.descricao);
        return ['todos', ...new Set(descriptions)];
    }, [transacoes]);
    ...
    const filteredTransacoes = useMemo(() => {
        ...
        return transacoes.filter(item => item.descricao === filter);
    }, [filter, transacoes]);
    ...
    return (
        <div className="max-w-4xl mx-auto p-4 sm:p-6">
            ...
            <div className="mb-6 flex items-end gap-4">
                ...
                <button onClick={() => generatePDF(filteredTransacoes, filter)} disabled={!isPdfLibReady} className="p-3 bg-blue-600 hover:bg-blue-500 rounded-lg transition-colors flex items-center gap-2 disabled:bg-gray-500 disabled:cursor-not-allowed">
                    <FileDown size={20} />
                    <span>{isPdfLibReady ? 'Relatório' : 'A carregar...'}</span>
                </button>
            </div>
            ...
            <div className="space-y-3">
                {filteredTransacoes.length > 0 ? filteredTransacoes.map(item => (
                    <div key={item.id} className="bg-gray-800 p-4 rounded-lg flex items-center">
                        <div className="flex-grow">
                            <p className="font-semibold text-white">{item.descricao}</p>
                            <p className="text-xs text-gray-400">{item.data?.toDate().toLocaleString('pt-BR') || item.createdAt?.toDate().toLocaleString('pt-BR') || 'Data indisponível'}</p>
                        </div>
                        ...
                        <button onClick={() => handleDeleteRequest(item.id, 'transacao')} className="text-gray-600 hover:text-red-500 transition-colors">
                            <Trash2 size={20} />
                        </button>
                    </div>
...
const CartoesContent = ({ cartoes, formatCurrency, handleDeleteRequest, isPdfLibReady }) => {
    ...
    const generatePDF = (items, filter) => {
        ...
        doc.text(`Total em Faturas: ${formatCurrency(totalFaturas)}`, 14, finalY + 16);

        doc.save(`relatorio_cartoes_${filter}.pdf`);
    };

    const filterOptions = useMemo(() => {
        const descriptions = cartoes.map(item => item.nome);
        return ['todos', ...new Set(descriptions)];
    }, [cartoes]);
    ...
    const filteredCartoes = useMemo(() => {
        ...
        return cartoes.filter(item => item.nome === filter);
    }, [filter, cartoes]);
    ...
    return (
        <div className="max-w-4xl mx-auto p-4 sm:p-6">
            <div className="bg-gray-800 rounded-2xl p-6 mb-6">
                <h3 className="text-lg font-bold text-white mb-4 text-center">Contabilidade de Cartões</h3>
                <div className="flex justify-center gap-12 text-center">
                    <div><p className="text-2xl font-bold text-blue-400">{formatCurrency(totalFaturas)}</p><p className="text-sm text-gray-400">Total em Faturas</p></div>
                </div>
            </div>
            ...
            <div className="mb-6 flex items-end gap-4">
                <div className="flex-grow">
                    ...
                    <select id="filter-cartoes" value={filter} onChange={(e) => setFilter(e.target.value)} className="w-full p-3 bg-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        {filterOptions.map(option => (
                            <option key={option} value={option}>{option === 'todos' ? 'Todos os Cartões' : option}</option>
                        ))}
                    </select>
                </div>
                <button onClick={() => generatePDF(filteredCartoes, filter)} disabled={!isPdfLibReady} className="p-3 bg-blue-600 hover:bg-blue-500 rounded-lg transition-colors flex items-center gap-2 disabled:bg-gray-500 disabled:cursor-not-allowed">
                    <FileDown size={20} />
                    <span>{isPdfLibReady ? 'Relatório' : 'A carregar...'}</span>
                </button>
            </div>
            ...
            <div className="space-y-3">
                {filteredCartoes.length > 0 ? filteredCartoes.map(card => (
                    <div key={card.id} className="bg-gray-800 p-4 rounded-lg flex items-center">
                        <div className="flex-grow">
                            <p className="font-semibold text-white">{card.nome}</p>
                            <p className="text-xs text-gray-400">Venc.: {new Date(Date.parse(card.vencimentoFatura + 'T00:00:00Z')).toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric', timeZone: 'UTC' })}</p>
                        </div>
                        <span className="font-bold text-lg mr-6 text-blue-400">{formatCurrency(card.valorFatura)}</span>
                        <button onClick={() => handleDeleteRequest(card.id, 'cartao')} className="text-gray-600 hover:text-red-500 transition-colors">
                            <Trash2 size={20} />
                        </button>
                    </div>
                )) : <p className="text-gray-500 text-center mt-8">Nenhum cartão encontrado.</p>}
            </div>
        </div>
    );
};
...
const PrestacoesContent = ({ prestacoes, formatCurrency, handleDeleteRequest, isPdfLibReady }) => {
    ...
    const generatePDF = (items, filter) => {
        ...
        const totalParcelasMes = items.reduce((acc, item) => acc + item.valorParcela, 0);
    ...
    const filterOptions = useMemo(() => {
        const descriptions = prestacoes.map(item => item.descricao);
        return ['todos', ...new Set(descriptions)];
    }, [prestacoes]);
    ...
    const filteredPrestacoes = useMemo(() => {
        ...
        return prestacoes.filter(item => item.descricao === filter);
    }, [filter, prestacoes]);
    ...
    return (
        <div className="max-w-4xl mx-auto p-4 sm:p-6">
            <div className="bg-gray-800 rounded-2xl p-6 mb-6">
                <h3 className="text-lg font-bold text-white mb-4 text-center">Contabilidade de Prestações</h3>
                <div className="flex justify-center items-end gap-12 text-center">
                    <div><p className="text-2xl font-bold text-yellow-400">{formatCurrency(prestacoesMesFiltradas)}</p><p className="text-sm text-gray-400">Total Parcelas do Mês</p></div>
                    <div><p className="text-base font-bold text-gray-300">{formatCurrency(totalDivida)}</p><p className="text-sm text-gray-400">Dívida Total</p></div>
                </div>
            </div>
            ...
            <div className="mb-6 flex items-end gap-4">
                <div className="flex-grow">
                    ...
                    <select id="filter-prestacoes" value={filter} onChange={(e) => setFilter(e.target.value)} className="w-full p-3 bg-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-yellow-500">
                        {filterOptions.map(option => (
                            <option key={option} value={option}>{option === 'todos' ? 'Todas as Prestações' : option}</option>
                        ))}
                    </select>
                </div>
                <button onClick={() => generatePDF(filteredPrestacoes, filter)} disabled={!isPdfLibReady} className="p-3 bg-yellow-600 hover:bg-yellow-500 rounded-lg transition-colors flex items-center gap-2 disabled:bg-gray-500 disabled:cursor-not-allowed">
                    <FileDown size={20} />
                    <span>{isPdfLibReady ? 'Relatório' : 'A carregar...'}</span>
                </button>
            </div>
            ...
            <div className="space-y-3">
                {filteredPrestacoes.length > 0 ? filteredPrestacoes.map(item => (
                    <div key={item.id} className="bg-gray-800 p-4 rounded-lg flex items-center">
                        <div className="flex-grow">
                            <p className="font-semibold text-white">{item.descricao}</p>
                            <p className="text-xs text-gray-400">{item.numParcelas} parcelas de {formatCurrency(item.valorParcela)}</p>
                        ...
                        <div className="text-right mr-6">
                            <p className="text-sm text-gray-400">Total</p>
                            <p className="font-bold text-lg text-yellow-400">{formatCurrency(item.valorTotal)}</p>
                        </div>
                        <button onClick={() => handleDeleteRequest(item.id, 'prestacao')} className="text-gray-600 hover:text-red-500 transition-colors">
                            <Trash2 size={20} />
                        </button>
                    </div>
                )) : <p className="text-gray-500 text-center mt-8">Nenhuma prestação encontrada.</p>}
            </div>
        </div>
    );
};
